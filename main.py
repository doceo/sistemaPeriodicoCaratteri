...
def get_resource(relative_path):
    ...
    return ImageTk.PhotoImage(Image.open(path.join(base_path, relative_path)))
...
class Value_Button(tk.Button):
    def __init__(self, number: int, *args, **kwargs):
        ...
        self.text = ""
        self.unclicked = get_resource(f"buttons/{self.value}_unclicked.png")
        self.clicked = get_resource(f"buttons/{self.value}_clicked.png")
...
class Image_Button(tk.Button):
    def __init__(self, button_name: str, *args, **kwargs):
        tk.Button.__init__(self, *args, **kwargs)
        self.text = ""
        self.button_image = get_resource(f"buttons/{button_name}.png")
...
class Final_Page(Page):
    def __init__(self, percentuali, labels, *args, **kwargs):
        ...
        self.wedges, _ = self.ax.pie(
            percentuali.values(), labels=get_symbols(), counterclock=False
        )
        ...
        self.text_1 = tk.Label(
            self,
            text="Basandosi sulle tue risposte, tu sei una lega composta da:",
            font=("Tahoma", 35),
        )
        ...
        if event.inaxes == self.ax:
            for w in self.wedges:
                if w.contains_point([event.x, event.y]):
                    ...
                    title = center_text(
                        f"{element[0]} - {round(self.percentuali[element[0]], 3)}%", 50
                    )
                    ...
                    w.set_center((0.2 * np.cos(theta), 0.2 * np.sin(theta)))
...
class Question_Page(Page):
    def __init__(self, caratteristica, domanda, page_number, *args, **kwargs):
        ...
        self.text_1.config(
            bg="white", wrap="word", relief="flat", state="disabled", width=45, height=3
        )

        self.value_buttons = []
        for i in range(1, 6):
            self.value_buttons.append(
                Value_Button(
                    int(i), self, command=partial(self.check_box, i), borderwidth=0
                )
            )
        ...
        for i in self.value_buttons:
            ...
            self.next_button = Image_Button(
                "next", self, command=self.next, borderwidth=0
            )
            ...
            self.text_3 = tk.Label(
                self, text=str(self.page_number) + "/12", font=("Tahoma", 30)
            )
...
class MainView(tk.Frame):
    def __init__(self, *args, **kwargs):
        ...
        self.pages = list(
            reversed(
                [
                    Question_Page(self.caratteristiche[i], self.questions[i], i + 1)
                    for i in range(len(self.caratteristiche))
                ]
            )
        )
        ...
        self.start_button = Image_Button(
            "start", self, command=self.show_pages, borderwidth=0
        )
        self.start_button.place(relx=0.5, rely=0.5, anchor="center")
        self.start_button.config(bg="white", width=540, height=161)

        self.credits_button = Image_Button(
            "credits", self, command=self.show_credits, borderwidth=0
        )
    ...
    def show_results(self):
        ...
        self.punteggi = {
            elemento: get_points(self.elementi[elemento], self.risposte)
            for elemento in self.elementi.keys()
        }
        totale_punti = sum(list(self.punteggi.values()))

        percentuali = {
            elemento: percentage(totale_punti, self.punteggi[elemento])
            for elemento in self.elementi.keys()
        }
